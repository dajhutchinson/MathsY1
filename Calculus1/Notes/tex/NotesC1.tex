...
So for a set of \textit{parametric equations}, $(x(t), y(t)),\ a \leq t \leq b$, $$\frac{ds}{dt} = \sqrt{\frac{dx^2}{dt} + \frac{dy^2}{dt}}$$
...
\underline{i.e.} - $y=a\frac{dx}{dy}+bx$.
...
A differential equation is said to be \textit{linear} if it can be written in the form $$Ay(x) := a_n.y^{(n)}(x) + ... + a_1.y'(x) + a_0.y(x) = b(x)$$
...
We now define the \textit{Summing Factor}, $S_n$, as $$S_n :=\prod_{j={n_0}}^{n-1}\frac{-1}{b_j}$$.
...
\underline{i.e} - So all values repeat at a regular interval in the system.
...
The equilibrium $\vect{x}$ is stable if the modulus of $\vect{f}'(\vect{X})$ is less than 1.\\
